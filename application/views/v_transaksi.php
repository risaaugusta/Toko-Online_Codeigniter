...
<div class="col-md-6">
	<table id="example" class="table table-hover table-striped">
		...
		<tbody>
			<?php $no=0; foreach($tampil_jannah as $jannah): $no++; ?>
			<tr>
				<td><?=$no?></td>
				...
				<?php if($jannah->stok>0): ?>
				<td><a href="<?=base_url('index.php/transaksi/addcart/'.$jannah->id_jannah)?>" class="btn btn-info">Pesan</a></td>
			<?php elseif($jannah->stok==0): ?>
	...
	<form action="<?=base_url('index.php/transaksi/simpan')?>" method="post">
		...
		<table class="table table-striped table-hover" blocked>
			<tr>
				...
				<input type="hidden" name="id_jannah[]" value="<?=$items['id']?>">
			...
			<tr>
				...
				<th><input type="number" name="kembali" class="form-control" style="display: inline-block;width:200px" value="<?= $this->session->flashdata('kenbali')?>"></th>
				<th></th>
			</tr>
		</table>
